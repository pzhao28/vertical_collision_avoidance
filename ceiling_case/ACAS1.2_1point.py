...
def state_cost(own_name, order, all_ac, T_to_hit, step, own_height):
    ...
    dc_idx[dc_idx >= 0] = 1
    dc_idx[dc_idx < 0] = 0
    st_cost = np.sum(np.exp(-abs(relative_height/185)) * np.exp(-abs(time_to_hit/60)) * dc_idx)
    return st_cost
...
# Initialize
T_to_hit = [[0,100,100],[100,0,100],[100,100,0]] #case1: three aircrafts hit in 3 points
...
ceiling_map = mat.repmat(np.append(np.ones(int(ceiling_height/height_range[2])), np.ones(len(non_ceiling)) * np.exp(-non_ceiling*2 / alpha)), interval + 1, 1)
