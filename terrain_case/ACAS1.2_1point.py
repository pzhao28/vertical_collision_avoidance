...
def main():
    ...
    while step < num_iteration:
        ...
        for key in order:
            ...
            sigma_i += np.sum((trvl_plan[0:interval] - pre_plan)**2)
...
class Grid:
    ...
    def cost_map(self, own_name, all_ac, T_to_hit):
        cst_data = np.array(self.data, dtype = np.float64)
        for i in range(len(self.x)):
            for j in range(len(self.y)):
                cst_data[i][j] = state_cost(own_name, all_ac, T_to_hit, i, self.y[j])
        return cst_data
...
def state_cost(own_name, all_ac, T_to_hit, step, own_height):
    intruder = dict()
    intruders_plan = []
    i = 0
    for key in all_ac:
        if key == own_name:
            hit_point = np.array(T_to_hit[i])
            hit_point = np.delete(hit_point, i)
            continue
        else:
    ...
    dc_idx[dc_idx >= 0] = 1
    dc_idx[dc_idx < 0] = 0
    st_cost = np.sum(np.exp(-abs(relative_height/alpha)) * np.exp(-abs(time_to_hit/beta)) * dc_idx)
    return st_cost
...
# Initialize
T_to_hit = {'ac1':[0,100,120,110,120],'ac2':[100,0,10000,120,110],'ac3':[120,10000,0,100,120],'ac4':[110,120,100,0,10000],'ac5':[120,110,120,10000,0]}
...
terrain_map = mat.repmat(np.append(np.ones(int(terrain_height/height_range[2])), np.ones(len(non_terrain)) * np.exp(-non_terrain / alpha)), interval + 1, 1)
