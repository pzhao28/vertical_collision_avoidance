...
def main():
    ...
    #for alpha in range(120, 480, 120):
    beta = 360
    for alpha in [40, 50, 60, 70, 80, 90]:
    #for alpha in [40]:
        sigma = alpha/np.sqrt(2)
        t = 0
        while t < 200:
            pdf = np.exp(-0.5*((200-t)/sigma)**2) / (np.sqrt(2*np.pi)*sigma)
            if np.exp(-0.5*((200-t)/sigma)**2) / (np.sqrt(2*np.pi)*sigma) > 5e-4:
                break
            t += 1
        start_state[2] = t
        ...
        step = 0
        i = 1
        print("\ralpha {}/90".format(alpha), end="")
        sys.stdout.flush()
        while step < num_iteration:
            ...
            for key in order:
                ...
                if step == 0:
                    cst_map = world.cost_map(key, order, all_ac, T_to_hit, alpha, beta/2)
                else:
                    del all_ac['ac_virtual']
                    cst_map = world.cost_map(key, order, all_ac, T_to_hit, alpha, beta)
                #else:
                ...
                if step == num_iteration - 1:
                    axs[i].plot((trvl_plan - start_state[0]) * height_range[2] * 50 / 6, label = r'$\alpha$={}'.format(alpha))
                    axs[i].set_title('Path Planning of Aircraft A' if key == 'ac2' else 'Path Planning of Aircraft B')
                    ...
                    '''if step == num_iteration - 1:
                        ...
                        axs[i, 1].plot((trvl_plan - start_state[0]) * height_range[2]*50/6)
                        axs[i, 1].set_title('Path Planning of Aircraft A' if i==0 else 'Path Planning of Aircraft B')
                        axs[i, 1].set_xlabel('Time(s)')
                        axs[i, 1].set_ylim(-700, 700)
                        i += 1'''
...
class Grid:
    ...
    def cost_map(self, own_name, order, all_ac, T_to_hit, alpha, beta):
        cst_data = np.array(self.data, dtype = np.float64)
        for i in range(len(self.x)):
            for j in range(len(self.y)):
                cst_data[i][j] = state_cost(own_name, order, all_ac, T_to_hit, i, self.y[j], alpha, beta)
        return cst_data
...
def state_cost(own_name, order, all_ac, T_to_hit, step, own_height, alpha, beta):
    intruder = dict()
    intruders_plan = []
    for key in all_ac:
        if key == own_name:
    ...
    st_cost = np.sum(np.exp(-(relative_height*(50/6)/beta)**2) * np.exp(-(time_to_hit/alpha)**2))
    return st_cost
...
def slv_mdp(start_state, cost_map):
    ...
    opt_trajectory = np.zeros([interval+1, 2], dtype = np.int) + start_state[0] # Output: optimal trajectory, (height, velocity), entries before start_state[2] is 0.
...
T_to_hit = {'ac1':[0,200,200],'ac2':[200,0,200], 'ac_virtual':[200,200,0]}
