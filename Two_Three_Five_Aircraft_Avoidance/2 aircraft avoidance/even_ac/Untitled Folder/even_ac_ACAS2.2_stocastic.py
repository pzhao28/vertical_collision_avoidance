...
#            all_ac.update(ac_virtual)
            ...
            if step == num_iteration - 1 and key == 'ac1':
                #plt_cst_map(axs[i, 0], cst_map)
                axis_font = {'size':'12'}
                fig1, axs1 = plt.subplots(1, 2, figsize=(10,4))
                im1 = axs1[0].imshow(np.transpose(cst_map), origin='lower', extent=[0, 200, -2000, 2000], aspect="auto")
                axs1[0].set_title('Cost Map of Aircraft A')
                axs1[0].set_xlabel('Time(s)', **axis_font)
                axs1[0].set_ylabel('Relative height(ft)', **axis_font)
                #plt_trvl_plan(axs[i, 1], trvl_plan)
                axs1[1].plot((trvl_plan - start_state[0]) * height_range[2]*50/6)
                axs1[1].set_title('Path Planning of Aircraft A')
                axs1[1].set_xlabel('Time(s)', **axis_font)
                axs1[1].set_ylim(-2000, 2000)
                fig1.colorbar(im1, ax = axs1[0])
                plt.tight_layout()
                plt.savefig('Final Results 1')
                plt.close()

            if step == num_iteration - 1 and key == 'ac2':
                axis_font = {'size': '12'}
                fig2, axs2 = plt.subplots(1, 2, figsize=(10,4))
                im2 = axs2[0].imshow(np.transpose(cst_map), origin='lower', extent=[0, 200, -2000, 2000], aspect="auto")
                axs2[0].set_title('Cost Map of Aircraft B')
                axs2[0].set_xlabel('Time(s)', **axis_font)
                axs2[0].set_ylabel('Relative height(ft)', **axis_font)
                # plt_trvl_plan(axs[i, 1], trvl_plan)
                axs2[1].plot((trvl_plan - start_state[0]) * height_range[2] * 50/6)
                axs2[1].set_title('Path Planning of Aircraft B')
                axs2[1].set_xlabel('Time(s)', **axis_font)
                axs2[1].set_ylim(-2000, 2000)
                fig2.colorbar(im2, ax=axs2[0])
                plt.tight_layout()
                plt.savefig('Final Results 2')
                plt.close()
...
# Initialize
T_to_hit = {'ac1':[0,100,100],'ac2':[100,0,100]}
