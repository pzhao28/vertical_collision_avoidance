...
def main():
    ...
    while step < num_iteration:
        ...
        for key in order:
            ...
            if step == 0:
                plt.savefig('first_iteration_{}'.format(key))
            elif step == num_iteration -1:
                plt.savefig('final_iteration_{}'.format(key))
            plt.close()
...
action_cost = np.array([0.04, 0.02, 0, 0.04, 0.02]) * num_ac/2
...
ac4 = ac.set_ac('ac4', 0, 0, init_plan)
ac5 = ac.set_ac('ac5', 0, 0, init_plan)
all_ac = {**ac1, **ac2, **ac3, **ac4, **ac5}
...
order = ['ac1', 'ac2', 'ac3', 'ac4', 'ac5'] #fixed order! Do not change exept adding or deleting.
axis_font = {'size':'12'}
