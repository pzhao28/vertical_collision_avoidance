...
def main():
    ...
    all_ac = {}
    T_to_hit = {}
    all_ac.update(uncooperative) # uncooperative aircraft
    ...
    while step < num_iteration:
        ...
        for key in order:
            ...
            if step == 0:
                all_ac.update({key: all_ac_init[key]})
                T_to_hit_uncooperative = np.array(T_to_hit_ac['uncooperative'])
                T_to_hit_key = np.array(T_to_hit_ac[key])
                T_to_hit_key_tmp = T_to_hit_key[np.isinf(np.array(T_to_hit_uncooperative + T_to_hit_key))]
                T_to_hit_uncooperative_tmp = T_to_hit_uncooperative[np.isinf(T_to_hit_uncooperative + T_to_hit_key)]
                T_to_hit = {key: T_to_hit_key_tmp, 'uncooperative': T_to_hit_uncooperative_tmp}
            else:
                T_to_hit = T_to_hit_ac


            #TODO1: define slv_mdp
            cst_map = world.cost_map(key, all_ac, T_to_hit, beta)
...
class Grid:
    ...
    def cost_map(self, own_name, all_ac, T_to_hit, beta):
        cst_data = np.array(self.data, dtype = np.float64)
        for i in range(len(self.x)):
            for j in range(len(self.y)):
                cst_data[i][j] = state_cost(own_name, all_ac, T_to_hit, i, self.y[j], beta)
        return cst_data
...
def state_cost(own_name, all_ac, T_to_hit, step, own_height, beta):
    intruder = dict()
    intruders_plan = []
    order_tmp = T_to_hit.keys()
    order = []
    for i in range(len(order_num)):
        if order_num[i] in order_tmp:
            order.append(order_num[i])
    for key in order:
        if key == own_name:
...
T_to_hit_ac1 = {'ac1':[np.inf, 100, 100]}
T_to_hit_ac2 = {'uncooperative':[100, np.inf, 0]}
T_to_hit_ac3 = {'ac3':[100, 0, np.inf]}
T_to_hit_ac = {**T_to_hit_ac1, **T_to_hit_ac2, **T_to_hit_ac3}
num_ac = 2
interval = 200
...
ac2 = ac.set_ac('uncooperative', start_ac2, 0, np.array([start_ac2]*interval))
ac3 = ac.set_ac('ac3', start_ac3, 0, np.array([start_ac3]*interval))
all_ac_init = {**ac1, **ac2, **ac3}
uncooperative = ac2
...
order_ac = ['ac3', 'ac1'] #fixed order! Do not change exept adding or deleting.
order_num = ['ac1', 'uncooperative', 'ac3']
