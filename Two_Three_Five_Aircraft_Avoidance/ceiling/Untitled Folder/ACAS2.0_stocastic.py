...
def main():
    ...
    while step < num_iteration:
        ...
        for key in order:
            ...
            if key == order[0]:
                cst_map = world.cost_map(key, order, all_ac, T_to_hit) + ceiling_map
            else:
                cst_map = world.cost_map(key, order, all_ac, T_to_hit)
            ...
            plt.close()'''
            #plt.show()
            plt_trvl_plan(ax[1], trvl_plan)
            if step == 0:
                plt.savefig('first_iteration{}'.format(key))
            elif step == num_iteration - 1:
                plt.savefig('final_iteration{}'.format(key))
            plt.close()
    ...
    #plt.plot(sigma)
    #plt.plot(total_cost)
    #plt.savefig('diff_path_discretization_{}'.format(height_range[2]) + '_iteration_{}'.format(num_iteration))
    #plt.close()
    #sio.savemat("conv.mat", {'foo':sigma})
...
def plt_cst_map(fig, ax, cst_map):
    im = ax.imshow(np.transpose(cst_map),origin='lower',extent=extent, aspect="auto")
    ax.set_xlabel("time(s)", **axis_font)
    ax.set_ylabel("relative height(ft)", **axis_font)
...
T_to_hit = {'ac1':[0,100,100],'ac2':[100,0,100],'ac3':[100,100,0]}
 #case1: three aircrafts hit in 3 points
...
ceiling_height = 300 * height_range[2]
non_ceiling = np.arange(0, (height_range[1] - height_range[0] - ceiling_height) + 1, height_range[2])
ceiling_map = mat.repmat(np.append(np.ones(int(ceiling_height/height_range[2])), np.ones(len(non_ceiling)) * np.exp(-(non_ceiling / (2*alpha))**2)), interval + 1, 1)
ceiling_map = np.fliplr(ceiling_map)
